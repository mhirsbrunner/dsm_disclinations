...
n = 5000 # dimension of the matrix
...
site = 0#n//2 # the entry where you want the dos, n/2 is an atom in the middle
scale = 2.2 # this number has to be such that max(|eigenvalues|)/scale < 1
npol = 3000 # number of polynomials, energy resolution goes as 1/npol
...
t0 = time.time()
ntries = 200 # number of stochastic vectors, with a few is fine
(x1,y1) = kpm.tdos0d(m,scale=scale,npol=npol,ne=ne,ntries=ntries)
t1 = time.time()
